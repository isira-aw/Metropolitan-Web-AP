...
export default function EditJobPositionPage({ params }: { params: { id: string } }) {
  ...
  const { data: jobPosition, isLoading } = useQuery({
    queryKey: ["job-position", id],
    queryFn: () => jobPositionsApi.getById(id),
  });
  ...
  useEffect(() => {
    if (jobPosition) {
      setFormData({
        ...
        detail: jobPosition.detail,
        category: jobPosition.category,
        information: jobPosition.information || "",
  ...
  const mutation = useMutation({
    mutationFn: (data: typeof formData) => jobPositionsApi.update(id, data),
    onSuccess: () => {
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="flex gap-4">
              <Button type="submit" disabled={mutation.isPending} className="flex-1">
                {mutation.isPending ? "Updating..." : "Update Job Position"}
              </Button>
